#!/usr/bin/env python3
"""
Donjon Platform - Tool Discovery & Management
...
class ToolDiscovery:
    ...
    def _check_local_tools_dir(self):
        ...
        # Check for nuclei
        candidates = ('nuclei.exe', 'nuclei') if is_windows else ('nuclei',)
        ...
        # Check for nmap portable — nmap.exe only works on Windows
        if is_windows:
        ...
        if gobuster_dir.exists():
            candidates = ('gobuster.exe', 'gobuster') if is_windows else ('gobuster',)
            for name in candidates:
        ...
        if amass_dir.exists():
            candidates = ('amass.exe', 'amass') if is_windows else ('amass',)
            for name in candidates:
        ...
        # Check for Sysinternals tools — Windows-only
        if is_windows:
    ...
    def _run_cmd(self, cmd: str) -> tuple:
        ...
        try:
            ...
            if _sys.platform != 'win32' and not any(c in cmd for c in ['|', '&&', '||', '>', '<', ';', '2>']):
                ...
                result = subprocess.run(
                    cmd,
                    shell=True,  # nosec B602 -- cmd is from internal ToolInfo.version_cmd, not user input
                    capture_output=True,
                    text=True,
                    timeout=10
                )
    ...
    def install_tool(self, name: str) -> bool:
        ...
        if _sys.platform == 'win32':
            if tool.install_cmd_windows:
                ...
                result = subprocess.run(tool.install_cmd_windows, shell=True)  # nosec B602 -- install_cmd from internal ToolInfo
        ...
        if tool.install_apt:
            if shutil.which('apt'):
                ...
                result = subprocess.run(
                    ...
                    shell=True  # nosec B602 -- install_apt from internal ToolInfo registry
                )
                if result.returncode == 0:
                    self.check_tool(name)
                    if tool.available:
        ...
        if tool.install_cmd:
            ...
            result = subprocess.run(tool.install_cmd, shell=True)  # nosec B602 -- install_cmd from internal ToolInfo
            if result.returncode == 0:
                self.check_tool(name)
                if tool.available:
                    tui.success(f"{tool.name} installed successfully")
