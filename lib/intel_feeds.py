#!/usr/bin/env python3
"""
Donjon Platform - Multi-Source Intelligence Feed Manager
...
class IntelFeedManager:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.code in (429, 503) and attempt < max_retries - 1:
                    ...
                    logger.warning(f"HTTP {e.code} from {url}, retrying in {wait}s...")
                    time.sleep(wait)
                    continue
                logger.error(f"HTTP {e.code} from {url}: {e.reason}")
                raise
            except (urllib.error.URLError, TimeoutError, OSError) as e:
                if attempt < max_retries - 1:
                    wait = (2 ** attempt) * 3
                    logger.warning(f"Network error fetching {url}, retrying in {wait}s: {e}")
                    time.sleep(wait)
                    continue
                raise
    ...
    def update_exploitdb(self) -> Dict[str, Any]:
        ...
        try:
            raw = self._fetch_with_retry(EXPLOITDB_CSV_URL, timeout=120)
    ...
    def _osv_fetch_ecosystem(self, conn: sqlite3.Connection,
        ...
        try:
            zip_data = self._fetch_with_retry(url, timeout=120)
        except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, OSError) as e:
    ...
    def update_github_advisories(self) -> Dict[str, Any]:
        ...
        while page <= max_pages:
            ...
            try:
                raw = self._fetch_with_retry(url, headers=headers, timeout=30)
    ...
    def update_cisa_alerts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raw = self._fetch_with_retry(CISA_KEV_URL, timeout=60)
            kev = json.loads(raw)
        ...
        try:
            ...
            raw = self._fetch_with_retry(CISA_ALERTS_RSS_URL, timeout=60,
    ...
    def _parse_cisa_rss(self, conn: sqlite3.Connection, xml_text: str) -> int:
        ...
        count = 0
        try:
            root = safe_fromstring(xml_text)  # nosec B314 -- using defusedxml when available
    ...
    def update_mitre_attack(self) -> Dict[str, Any]:
        ...
        try:
            raw = self._fetch_with_retry(MITRE_ATTACK_URL, timeout=180)
        except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, OSError) as e:
    ...
    # ==================================================================
    def _get_abusech_api_key(self) -> str:
        """Get abuse.ch API key from environment or config.

        Free API key available at https://auth.abuse.ch/
        Set ABUSECH_API_KEY environment variable or configure in settings.
        """
    ...
    def update_abuse_ch(self) -> Dict[str, Any]:
        ...
        Set ABUSECH_API_KEY env var to enable.
        Stores in threat_indicators table.
        Returns summary dict.
        """
        print("  [abuse.ch] Fetching threat feeds...")

        api_key = self._get_abusech_api_key()
        if not api_key:
            msg = ("  [abuse.ch] SKIPPED - no API key configured. "
                   "Get a free key at https://auth.abuse.ch/ "
                   "and set ABUSECH_API_KEY environment variable.")
            print(msg)
            logger.warning("abuse.ch feeds skipped: ABUSECH_API_KEY not set")
            return {"status": "skipped", "error": "No API key (ABUSECH_API_KEY)", "count": 0}
        ...
        try:
            ...
            url_with_key = f"{URLHAUS_API_URL}?auth-key={api_key}"
            req = urllib.request.Request(url_with_key, data=payload)
            req.add_header("User-Agent", "Donjon/1.0")
            ...
            if e.code == 401:
                print("AUTH FAILED - check ABUSECH_API_KEY")
                logger.error("URLhaus: 401 Unauthorized - API key invalid or expired")
            else:
                print(f"HTTP {e.code} ({e.reason})")
                logger.error(f"URLhaus fetch error: HTTP {e.code} {e.reason}")
        except (urllib.error.URLError, TimeoutError, OSError) as e:
            print(f"FAILED ({e})")
            logger.error(f"URLhaus network error: {e}")
        ...
        try:
            ...
            if e.code == 401:
                print("AUTH FAILED - check ABUSECH_API_KEY")
                logger.error("ThreatFox: 401 Unauthorized - API key invalid or expired")
            else:
                print(f"HTTP {e.code} ({e.reason})")
                logger.error(f"ThreatFox fetch error: HTTP {e.code} {e.reason}")
        except (urllib.error.URLError, TimeoutError, OSError) as e:
            print(f"FAILED ({e})")
            logger.error(f"ThreatFox network error: {e}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for table in tables:
            try:
                row = conn.execute(f"SELECT COUNT(*) FROM {table}").fetchone()  # nosec B608 -- table name from hardcoded allowlist, not user input
