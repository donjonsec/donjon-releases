...
# ---------------------------------------------------------------------------
# DonjonAPI  --  core API logic (framework-agnostic)
# ---------------------------------------------------------------------------
...
class DonjonAPI:
    ...
    def _register_routes(self):
        ...
        self._get('/api/v1/ai/config', self._ai_get_config)
    ...
    def _health(self, **kw) -> Tuple[bytes, int, str]:
        ...
        return json_response({
            'status': 'healthy',
            'version': _config.version if _config else '7.5.0',
    ...
    def _test_notification(self, body: Optional[Dict], **kw) -> Tuple[bytes, int, str]:
        ...
        try:
            nm._dispatch(
                channel,
                'Test Notification',
                'This is a test notification from Donjon.',
                'INFO',
            )
    ...
    def _list_scanners(self, **kw) -> Tuple[bytes, int, str]:
        ...
        scanners = [
            ...
            {'id': 'malware', 'name': 'Malware Scanner',
             ...
             'scan_depths': ['quick', 'standard', 'deep']},
            {'id': 'shadow_ai', 'name': 'Shadow AI Scanner',
             'description': 'Detect unauthorized AI: local LLMs, web AI, browser extensions, API keys, model files',
             'scan_depths': ['quick', 'standard', 'deep']},
    ...
    def _activate_license(self, body: Optional[Dict], **kw) -> Tuple[bytes, int, str]:
        ...
        # Accept v2 (dual signatures) or v1 (legacy HMAC) license payload
        if 'tier' in body and ('signatures' in body or 'signature' in body):
            license_data = body
        else:
            return error_response(
                'Invalid license format. Provide a signed license JSON from the Donjon license admin tool.', 400
            )
    ...
    def _network_local_info(self, **kw) -> Tuple[bytes, int, str]:
        ...
        # Enrich with psutil network interface data (netmask, interface names)
        try:
                ...
                for addr in addrs:
                    ...
                    if ip in [i['ip'] for i in info['interfaces']]:
                        ...
                        for entry in info['interfaces']:
                            if entry['ip'] == ip:
                                entry.setdefault('netmask', netmask)
                                entry.setdefault('interface', iface)
                        continue
                    parts = ip.split('.')
                    subnet = '.'.join(parts[:3]) + '.0/24'
                    info['interfaces'].append({
    ...
    def _ai_get_config(self, **kw) -> Tuple[bytes, int, str]:
        """Return current AI configuration (with masked keys)."""
        if not get_ai_engine:
            return json_response({'backend': 'none', 'available': False})
        try:
    ...
    def _ai_save_config(self, body: Optional[Dict], **kw) -> Tuple[bytes, int, str]:
        """Save AI configuration and reconfigure the engine."""
        if not get_ai_engine:
            return error_response('AI engine module not available', 503)
        if not body:
            return error_response('Request body required')
        try:
            engine = get_ai_engine()
            ...
            if get_audit_trail:
                get_audit_trail().log('ai_config_updated', target_type='ai',
                                     details={'backend': body.get('backend', '')})
            return json_response({
                'saved': True,
    ...
    def _ai_test_connection(self, **kw) -> Tuple[bytes, int, str]:
...
class DonjonHTTPHandler(BaseHTTPRequestHandler):
    """HTTP request handler backed by DonjonAPI."""

    api: DonjonAPI = None  # set by server factory
    ...
    def _parse_request(self) -> Tuple[str, Dict[str, str], Optional[Dict]]:
        ...
        if content_length > max_request_size:
            ...
            msg = json.dumps({'error': 'Request entity too large', 'max_bytes': max_request_size}).encode()
    ...
    def _get_api_key(self) -> Optional[str]:
        ...
        Query parameter authentication was removed to prevent key leakage
...
def create_stdlib_server(host: str = '0.0.0.0', port: int = 8443,  # nosec B104 -- intentional bind-all for server
                         auth: Optional[APIKeyAuth] = None) -> HTTPServer:
    """Create an HTTPServer using stdlib only (zero dependencies)."""
    api = DonjonAPI(auth=auth)
    DonjonHTTPHandler.api = api
    server = HTTPServer((host, port), DonjonHTTPHandler)
    return server
...
def start_server(host: str = '0.0.0.0', port: int = 8443,  # nosec B104 -- intentional bind-all for server
                 no_auth: bool = False, prefer_flask: bool = True,
                 tls_cert: str = None, tls_key: str = None):
    """
    Start the Donjon web server.

    Tries Flask first (if installed and preferred), falls back to stdlib.
    If tls_cert and tls_key are provided, serves HTTPS.
    """
    import ssl as _ssl
    ...
    if tls_cert and tls_key:
        if not os.path.exists(tls_cert):
            print(f"[Donjon] ERROR: TLS cert not found: {tls_cert}", file=sys.stderr)
            sys.exit(1)
        if not os.path.exists(tls_key):
            print(f"[Donjon] ERROR: TLS key not found: {tls_key}", file=sys.stderr)
            sys.exit(1)
    ...
    if flask_app is not None:
        print(f"[Donjon] Starting Flask server on {host}:{port}")
        print(f"[Donjon] Dashboard: {scheme}://{host}:{port}/")
        print(f"[Donjon] API Base:  {scheme}://{host}:{port}/api/v1/")
        print(f"[Donjon] TLS: {'enabled' if ssl_ctx else 'disabled'}")
        print(f"[Donjon] Auth: {'enabled' if auth.enabled else 'DISABLED'}")
        flask_app.run(host=host, port=port, debug=False,
                      ssl_context=ssl_ctx)
    else:
        print(f"[Donjon] Starting stdlib HTTP server on {host}:{port}")
        print(f"[Donjon] (Flask not found - using http.server)")
        print(f"[Donjon] Dashboard: {scheme}://{host}:{port}/")
        print(f"[Donjon] API Base:  {scheme}://{host}:{port}/api/v1/")
        print(f"[Donjon] TLS: {'enabled' if ssl_ctx else 'disabled'}")
        print(f"[Donjon] Auth: {'enabled' if auth.enabled else 'DISABLED'}")
