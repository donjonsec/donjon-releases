#!/usr/bin/env python3
"""
Donjon - Credentialed/Authenticated Scanner
...
class CredentialScanner(BaseScanner):
    ...
    def _get_ssh_client(self, target: str, cred: Dict):
        ...
        try:
            ...
            # nosec B507 - AutoAddPolicy is intentional: security scanner must
            # connect to arbitrary hosts to test SSH credential exposure.
    ...
    def _ssh_exec(self, client, command: str, target: str, cred: Dict) -> str:
        ...
        if client is not None:
            # paramiko client
            # nosec B601 - exec_command is intentional: credential scanner must
            # execute commands on remote hosts to verify access scope.
            _, stdout, stderr = client.exec_command(command, timeout=30)  # nosec B601
