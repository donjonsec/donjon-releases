#!/usr/bin/env python3
"""
Donjon v7.0 - Production Readiness Test Suite
...
# ---------------------------------------------------------------------------
# Path bootstrap - mirrors bin/donjon-launcher so every lib/scanner import works
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT / 'lib'))
sys.path.insert(0, str(PROJECT_ROOT / 'scanners'))
sys.path.insert(0, str(PROJECT_ROOT / 'utilities'))
os.environ.setdefault('DONJON_HOME', str(PROJECT_ROOT))
...
class TestPlatformDetect(unittest.TestCase):
    """Validate platform detection on current OS."""
    ...
    def test_is_windows(self):
        import sys
        self.assertEqual(self.pi.is_windows, sys.platform == 'win32')

    def test_os_name(self):
        import sys
        expected = 'Windows' if sys.platform == 'win32' else 'Linux' if sys.platform.startswith('linux') else 'macOS'
        self.assertEqual(self.pi.os_name, expected)
