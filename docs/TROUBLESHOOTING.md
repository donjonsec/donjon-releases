# Donjon Platform v7.0 - Troubleshooting Guide

Common issues and solutions for the Donjon Platform.
...
1. Run the platform as root/administrator (required for SYN scans and ICMP):
   ```bash
   sudo python3 bin/donjon-launcher quick
   ```
...
1. Ensure no other `donjon-launcher` processes are running:
   ```bash
   ps aux | grep donjon-launcher
...
1. Launch the platform: `python3 bin/donjon-launcher`
...
```bash
cp -r /media/usb/donjon/ /opt/donjon/
cd /opt/donjon/
