...
Version 7.0 transforms the Donjon Platform from a vulnerability management tool into a complete security intelligence platform. Ten new modules add FAIR risk quantification, AI-powered analysis, container and cloud security, SBOM generation, attack surface management, executive dashboards, CI/CD pipeline integration with SARIF output, adversary emulation with 35 threat actor profiles, and enterprise Linux compatibility (RHEL/SUSE).
...
## Latest: Enterprise Linux + Adversary Emulation (2026-02-16)

### Adversary Emulation Scanner

**File:** `scanners/adversary_scanner.py` (1726 lines)

Purple team defense validation inspired by Permiso's autonomous adversary simulation. Simulates known threat actor TTPs and scores your defensive coverage.

- 35 adversary profiles across 10 categories:
  - **Ransomware** (7): LockBit, Akira, BlackCat/ALPHV, Cl0p, Play, Rhysida, Qilin
  - **China APT** (4): Volt Typhoon, Salt Typhoon, APT40, APT41
  - **Russia APT** (4): APT29, APT28, Sandworm, Turla
  - **Iran APT** (3): APT33, APT34, CyberAv3ngers
  - **DPRK** (2): Lazarus, Kimsuky
  - **ICS/OT** (3): XENOTIME (Triton), CHERNOVITE (COSMICENERGY), ICS general
  - **Financial** (1): FIN7
  - **Hacktivist** (1): Z-Pentest/NoName057
  - **AI/LLM** (4): Prompt injection, Copilot compromise, agent hijack, supply chain
  - **Technique-specific** (5): Golden SAML, Kerberoast, container escape, supply chain, LOLBins
- 22 safe simulation methods (check defenses, never exploit)
- Scoring: DETECTED / PARTIAL / NOT_DETECTED with letter grades A-F
- Scorecard with ATT&CK technique IDs and kill chain correlation
- 10 YAML override profiles in `config/adversary_profiles/`
- Orchestrator Phase 10 integration (deep assessments only)
- Launcher Red Team menu + CLI: `donjon-launcher adversary`

### Enterprise Linux Compatibility

- **Build dependency pre-checks** (`bin/setup.sh`): Detects distro, checks gcc/Python.h/OpenSSL, auto-installs for Debian/RHEL/SUSE/Arch/macOS
- **SELinux/AppArmor detection** (`scanners/linux_scanner.py`): MAC framework checks, risky boolean audit
- **SUSE zypper scanning** (`scanners/linux_scanner.py`): Pending updates, security patches, cache freshness
- **RHEL CIS benchmarks** (`config/frameworks/rhel_cis.yaml`): 55 safeguards across 7 control families for RHEL 9/CentOS Stream 9/AlmaLinux 9/Rocky Linux 9
...
- Accessible from Compliance & Reports menu (option `r`) and CLI (`donjon-launcher risk`)
...
- Accessible from main menu (option `d`) and CLI (`donjon-launcher dashboard`)
...
| File | Changes |
|------|---------|
| `bin/donjon-launcher` | v7.0 menus, CLI commands (risk, dashboard), AI/SBOM/CI-CD handlers |
...
| Capability | Tenable.io | Qualys | RiskLens | Drata | Donjon v7.0 |
|-----------|------------|--------|----------|-------|-------------------|
...
python3 bin/donjon-launcher tools
