# Donjon - Docker Deployment Guide

This directory contains Docker containerization files for deploying Donjon in a multi-container environment with PostgreSQL backend.
...
### Backup Database
```bash
docker-compose exec postgres pg_dump -U donjon donjon > backup.sql
```
...
docker-compose exec postgres pg_isready -U donjon

# Check connection string
docker-compose exec api env | grep DONJON_DB
```

### API returns 401 Unauthorized
- Verify `DONJON_API_KEYS` is set in `.env`
...
docker run -it --rm \
  -v $(pwd)/data:/app/data \
  donjon-scanner \
  python /app/bin/donjon-launcher --non-interactive standard
```
...
| `POSTGRES_USER` | PostgreSQL username | `donjon` |
| `POSTGRES_DB` | PostgreSQL database name | `donjon` |
| `DONJON_DB_BACKEND` | Database backend (`sqlite` or `postgres`) | `postgres` |
| `DONJON_DB_URL` | PostgreSQL connection URL | Auto-generated |
| `NVD_API_KEY` | NVD API key for higher rate limits | None |
| `ABUSECH_API_KEY` | abuse.ch API key for URLhaus + ThreatFox | None |
| `DONJON_API_KEYS` | Comma-separated list of API keys | None |
